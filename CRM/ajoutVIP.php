<html>
    ...
    <body>
        ...
        if(isset($_POST["nom"]) and isset($_POST["prenom"]) and isset($_POST["typeVIP"]) //
        //sand isset($_POST["evaluation"]) and 
        //isset($_POST["description"])
         and isset($_POST["dateNaissance"]) and isset($_POST["nationalite"])and isset($_POST["classement"])){
            ...
            elseif($typeVIP == "autre"){
                ...
                $query->execute($data);
                /*
                $query = $bdd->prepare("INSERT INTO ListeVIP (IdAutreVIP, notation, Description) VALUES (?, ?, ?)");
                $data=array($,$prenom,$dateNaissance,$nationalite);
                $query->execute($data);*/
                echo"<p style=\"color:green\">Nouveau VIP ajouté</p>";
                //requete =
            }
            elseif($typeVIP == "arbitre"){
                $query = $bdd->prepare("INSERT INTO Arbitres (nom, prenom, dateNaissance, nationalite) VALUES (?,?,?,?)");
                $data=array($nom,$prenom,$dateNaissance,$nationalite);
                $query->execute($data);
    ...
    ?>    
        <form method='post' action="./ajoutVIP.php">
            ...
            <div>
            <input type="radio" id="abitre" name="typeVIP" value="arbitre">
            <label for="autreVIP">Arbitre (ne marche pas encore)</label>
            </div>

            <div>
            <input type="radio" id="ramasseur" name="typeVIP" value="ramasseur">
            <label for="autreVIP">Ramasseur de balles (ne marche pas encore)</label>
            </div>
